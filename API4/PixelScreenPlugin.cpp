                                            ...
                                            std::size_t color_count)
{
    ...
    if (zone_idx >= target.controller->zones.size()) return;

    zone& controller_zone = target.controller->zones[zone_idx];
    if (!controller_zone.matrix_map || !controller_zone.matrix_map->map) return;

    const unsigned int matrix_w  = controller_zone.matrix_map->width;
    const unsigned int matrix_h  = controller_zone.matrix_map->height;
    const unsigned int* map      = controller_zone.matrix_map->map;
    const unsigned int start_idx = controller_zone.start_idx;

    if (matrix_w == 0 || matrix_h == 0) return;
